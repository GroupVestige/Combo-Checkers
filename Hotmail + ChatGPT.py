...
# ASCII art for ChatGPT Subscription Checker
ascii_art = """
...
╚█████╔╝██║░░██║██║░░██║░░░██║░░░╚██████╔╝██║░░░░░░░░██║░░░  ╚█████╔╝██║░░██║███████╗╚█████╔╝██║░╚██╗███████╗██║░░██║
░╚════╝░╚═╝░░╚═╝╚═╝░░╚═╝░░░╚═╝░░░░╚═════╝░╚═╝░░░░░░░░╚═╝░░░  ░╚════╝░╚═╝░░╚═╝╚══════╝░╚════╝░╚═╝░░╚═╝╚══════╝╚═╝░░╚═╝
          Hotmail ChatGPT Subscription Checker
"""
...
def check_hotmail_account(email, password, output_file, full_capture_file):
    ...
    try:
        ...
        if response.status_code == 200 and "Sign in" not in response.text:
            ...
            # Check for ChatGPT subscription details
            chatgpt_subscription_exists = check_chatgpt_subscription(session)
            if chatgpt_subscription_exists:
                with open(full_capture_file, "a") as fc:
                    fc.write(f"{email}:{password} - ChatGPT Subscription\n")
                return "hit"
            else:
                print(f"No ChatGPT Subscription found for {email}")
                return "bad"
        else:
...
def check_chatgpt_subscription(session):
    # Example of checking ChatGPT subscription details after login
    # Replace with actual logic to check ChatGPT subscription existence
    # This is a placeholder function
    # Example: Check if the user has an active ChatGPT subscription
    return False
...
def main():
    ...
    output_file = input("Enter output file name (e.g., hotmail_success.txt): ").strip()
    full_capture_file = "hotmail_full_capture.txt"
